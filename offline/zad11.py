# Błażej Nowicki
# Wstawianie:
# Znajdujemy miejsce w drzewie gdzie należy wstawić nową wartość idąc odpowiednio w lewo albo w prawo. A następnie wstawiamy tworząc nowy węzeł i przepinając wskaźniki
# Usuwanie:
# Znajdujemy element w drzewie który chcemy usunąć. Jeśli to liść to ustawiamy wskaźnik węzła wyżej. Jeśli od węzła odchodzi tylko jedna gałąź to ją przepinamy
# Jeśli od węzłą odchodzą dwie gałęzie to szukamy poprzenika i zaminiamy watościami. Poprzednika usuwamy przpinając.
# Złożoność: O(n) (chyba że założymy że drzewo jest zbalansowane, wtedy logn)
